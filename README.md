...
See [`examples/`](examples/) for the full sample corpus — four demo campaigns (coffee launch, streetwear drop, wellness lineup, festival energy drink) each with a `brief.json`, a natural-language seed prompt, the autogenerated DALL-E prompts, and a walkthrough script.
