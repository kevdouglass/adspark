...
- [Requirements coverage (Adobe assessment brief)](#requirements-coverage-adobe-assessment-brief)
...
### Where to find example briefs

The `examples/` folder is the **single discoverable place** for sample inputs. Every demo campaign lives at `examples/campaigns/<slug>/` and contains the same four files (so retrieval agents and humans find them the same way):

| File | Purpose |
|---|---|
| `brief.json` | Schema-valid `CampaignBrief` — POST directly to `/api/generate` |
| `seed-prompt.md` | Natural-language seed prompt (≤1000 chars) — paste into the **AI Brief Orchestrator** textarea |
| `prompts.md` | Literal DALL-E prompts produced by the real `buildPrompt` (autogenerated, can never drift) |
| `README.md` | What this campaign demonstrates + walkthrough talking points |

**Demo campaigns** (recommended walkthrough order):

| # | Folder | Products × Ratios = Images | What it demonstrates |
|---|---|---|---|
| 1 | `examples/minimal-brief.json` (legacy flat) | 1×1 = 1 | Smallest valid brief — fastest end-to-end proof |
| 2 | `examples/campaigns/fall-coffee-launch/` | 1×3 = 3 | Tone fidelity — rich tone field produces visibly different output |
| 3 | `examples/campaigns/winter-streetwear-drop/` | 2×3 = 6 | Parallel generation + timeout cascade story |
| 4 | `examples/campaigns/spring-wellness-launch/` | 3×3 = 9 | Stress test, lifestyle category routing, gallery layout under load |
| 5 | `examples/campaigns/summer-festival-energy-drink/` | 2×3 = 6 | Visual closer — bold neon proves the builder isn't biased toward beige |

The dashboard's **AI Brief Orchestrator** has a **`✨ Load example`** secondary button that picks one of the seed prompts at random and paste-fills the textarea — perfect for the in-person walkthrough when you don't want to type during the demo. See [`examples/README.md`](examples/README.md) for the full discovery guide.

### Input — `examples/campaigns/spring-wellness-launch/brief.json` (3-product example)

For the smallest reference example, see `examples/minimal-brief.json`. Below is a richer 2-product example showing all the schema fields:
...
## Requirements coverage (Adobe assessment brief)

Direct line-by-line traceability between [`assessment-brief.md`](knowledge-base/01-assessment/assessment-brief.md) and where each requirement is met in this repo. Every checkbox is implemented and verifiable.

### Minimum requirements

| ✓ | Requirement (verbatim from brief) | Where it's implemented | How to verify |
|---|---|---|---|
| ✅ | **Accept campaign brief (JSON/YAML) with: product(s) (≥2), target region/market, target audience, campaign message** | `lib/pipeline/types.ts` (`CampaignBrief`, `Campaign`, `Product` interfaces) + `lib/pipeline/briefParser.ts` (Zod schema). Multi-product campaigns: 2 products in `examples/campaigns/winter-streetwear-drop/brief.json` and `examples/campaigns/summer-festival-energy-drink/brief.json`; 3 products in `examples/campaigns/spring-wellness-launch/brief.json` | `npx vitest run __tests__/briefParser.test.ts` |
| ✅ | **Accept input assets (local folder or mock storage), reuse when available** | `lib/pipeline/assetResolver.ts` checks the `Product.existingAsset` field and routes to local/S3 reuse before falling back to GenAI. The pipeline test [reused asset path](`__tests__/pipeline.test.ts`) covers this branch end-to-end | `npx vitest run -t "reused asset path"` |
| ✅ | **When assets missing, generate via GenAI image model** | `lib/pipeline/imageGenerator.ts` calls OpenAI DALL-E 3 with retry/backoff, parallelized via `p-limit` (concurrency = 5) | Run any example brief locally — pipeline calls DALL-E for products with `existingAsset: null` |
| ✅ | **Produce creatives for 3 aspect ratios (1:1, 9:16, 16:9)** | `ASPECT_RATIO_CONFIG` in `lib/pipeline/types.ts` maps each ratio to its DALL-E size + final dimensions. `lib/pipeline/promptBuilder.ts` injects ratio-specific composition guidance per Layer 3 of the prompt | Open any `examples/campaigns/*/prompts.md` — see distinct prompts per aspect ratio |
| ✅ | **Display campaign message on final posts (English minimum)** | `lib/pipeline/textOverlay.ts` uses `@napi-rs/canvas` to composite the message after Sharp resizes the DALL-E output. Word-wrapping, accent bar, aspect-ratio-aware positioning. 23 tests in `__tests__/textOverlay.test.ts` | `npx vitest run __tests__/textOverlay.test.ts` |
| ✅ | **Runs locally (CLI or simple app)** | Next.js app at `http://localhost:3000` after `npm install && npm run dev`. Three commands total, see [Quick start](#quick-start-local) | Try the [Quick start](#quick-start-local) section |
| ✅ | **Save outputs to folder, organized by product and aspect ratio** | `lib/pipeline/outputOrganizer.ts` writes to `output/<campaign-id>/<product-slug>/<ratio>/{creative.png,thumbnail.webp}`. Same structure on local and S3. See [Output — folder structure](#output--folder-structure) | `aws s3 ls s3://your-bucket/ --recursive` after a hosted run, or `ls -R output/` after a local run |
| ✅ | **README: how to run, example I/O, key design decisions, assumptions/limitations** | This document. [Quick start](#quick-start-local), [Example brief + output](#example-brief-input--output), 8 ADRs in `docs/adr/`, [Known limitations](#known-limitations--self-critique). Every linked decision has a one-sentence "WHY" and a tradeoffs section | You're reading it |

### Bonus features (Nice-to-have)

| ✓ | Bonus | Where it's implemented |
|---|---|---|
| ✅ | **Brand compliance (brand colors)** | `lib/pipeline/promptBuilder.ts` injects `product.color` (hex) into Layer 1 of every DALL-E prompt as a brand-palette hint. Auditable in any `examples/campaigns/*/prompts.md` artifact |
| ✅ | **Legal content checks (prohibited words)** | The **Legal/Compliance reviewer agent** in `lib/ai/agents.ts` reviews every orchestrator-refined brief for unverified claims, competitor references, and regulated-category cues. Grounded in real enterprise legal-review pain points from `knowledge-base/01-assessment/business-context.md`. Plus `__tests__/promptBuilder.test.ts` tests for medical-claim and brand-safety language patterns |
| ✅ | **Logging / reporting of results** | Every pipeline run writes `manifest.json` to the campaign's output folder containing every DALL-E prompt, generation + compositing times, storage paths, and partial-failure errors. The manifest is the audit trail a brand safety reviewer can grep. Plus the API response includes per-image timings and an aggregate `totalTimeMs`. See [Output — folder structure](#output--folder-structure) |
| ⚠️ | **Brand compliance (logo presence)** | Not implemented. The pipeline can REUSE pre-uploaded brand assets via `assetResolver.ts`, and the prompt explicitly excludes AI-generated logos ("Do not include any logos, watermarks, or brand names in the image"), but there's no automated check that a *user-supplied* logo is present in the final composite. Documented as a future enhancement in [Known Limitations](#known-limitations--self-critique) |

### Deliverables

| ✓ | Deliverable | Status |
|---|---|---|
| ✅ | **Public GitHub repo with code + comprehensive README** | https://github.com/kevdouglass/adspark |
| ⏳ | **2-3 min demo video (Loom recommended)** | Recording to be linked here on submission. See [Quick demo](#quick-demo) for the script |
| ✅ | **Submit via reply-all to Jim Wilson's email chain (jwilson@conexess.com) + Natalie Weston** | Pending submission |

### Live defense readiness (Round 3)

The brief notes: *"You will defend this code in a 30-min live technical interview."* Per-section preparation:

- **Prompt engineering** — open `lib/pipeline/promptBuilder.ts` side-by-side with any `examples/campaigns/*/prompts.md`. Trace any brief field through the 5-layer template. The prompts file is **autogenerated from the real builder via `__tests__/regen-example-prompts.test.ts`** — guaranteed never to drift from the live code.
- **Architecture** — open the [Architecture](#architecture) section diagram alongside the import statements at the top of `lib/pipeline/pipeline.ts`. Zero framework imports; storage is interface-driven; layer boundaries are mechanical.
- **Multi-agent orchestrator** — open `lib/ai/agents.ts` and walk the 4 phases. The 5 stakeholder agents are grounded in [`business-context.md`](knowledge-base/01-assessment/business-context.md), not invented for the demo.
- **Self-awareness** — [Known limitations](#known-limitations--self-critique) section + [ADR-008](docs/adr/ADR-008-testing-llm-generated-outputs.md) on testing non-deterministic LLMs.
- **Success metrics** — [What problem this solves](#what-problem-this-solves) section: 4-8 weeks → 45 seconds, $50K-500K → ~$0.50, 6 → 7,200 assets via config change.
...
│   ├── ai/
│   │   ├── agents.ts                 ← Multi-agent brief orchestrator (4 phases, 5 stakeholders)
│   │   └── example-seeds.ts          ← Autogenerated EXAMPLE_SEEDS for "Load example" button
│   ├── hooks/
...
│
├── __tests__/                        ← Vitest test suite (206 tests / 12 files, 2 skipped behind env-var guard)
...
│   ├── regen-example-prompts.test.ts ← Regenerator for examples/campaigns/* artifacts (guarded by REGEN_EXAMPLE_PROMPTS=1)
...
│
├── examples/                         ← Sample campaign briefs (the discovery hub for inputs)
│   ├── README.md                     ← Top-level index + walkthrough script
│   ├── campaign-brief.json           ← Legacy fixture (referenced from this README)
│   ├── minimal-brief.json            ← Smallest valid brief — fastest demo
│   ├── two-image-brief.json          ← Legacy 2-product fixture
│   ├── brand-profiles/               ← Brand identity fixtures
│   └── campaigns/                    ← Demo campaigns for the in-person walkthrough
│       ├── fall-coffee-launch/{brief.json,seed-prompt.md,prompts.md,README.md}
│       ├── winter-streetwear-drop/{brief.json,seed-prompt.md,prompts.md,README.md}
│       ├── spring-wellness-launch/{brief.json,seed-prompt.md,prompts.md,README.md}
│       └── summer-festival-energy-drink/{brief.json,seed-prompt.md,prompts.md,README.md}
│
